...
+
[[note:http-retries]]
If HTTP calls to that repository fail, Gradle will retry 3 times starting at 1s and increasing the amount of time between each call. The number of attempts and length of time between attempts is not configurable.
